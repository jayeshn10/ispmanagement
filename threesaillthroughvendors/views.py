...
from threesaillthroughvendors.filters import TTVIllFeasibleConnDetailsFilter
from threesaillthroughvendors.forms import AddIllConForm, AddSalesDetailsForm, EditSalesDetailsForm, \
    AddNocDetailsForm, EditNocDetailsForm, AddFieldEnggDetailsForm, EditFieldEnggDetailsForm, AddBillingDetailsForm, \
    EditBillingDetailsForm, RenewBillingDetailsForm
...
def ttvillconnections(request):
    ...
    objfeasible = TTVIllDoneConnectionsDetails.objects.all()

    connfilter = TTVIllFeasibleConnDetailsFilter(request.GET, queryset=objfeasible)
    objfeasible = connfilter.qs

    if request.method == 'GET' and request.GET.get('searchqry'):
        searchguery = request.GET.get('searchqry')
        objfeasible = TTVIllDoneConnectionsDetails.objects.filter(Q(conn_code__ill_custid__icontains=searchguery)
                                                                  | Q(
    ...
    if request.method == 'POST' and request.POST.get('exportexcel'):
        ...
        response['Content-Disposition'] = 'attachment; filename="TTV-Ill-done-Connections.xls"'

        wb = xlwt.Workbook(encoding='utf-8')
        ws = wb.add_sheet('TTV Ill Done Connections')  # this will make a sheet named Users Data
        ...
        ws.write_merge(0, 0, 24, 37, 'NOC details', font_style)
        ws.write_merge(0, 0, 38, 45, 'Field Engineer details', font_style)
        ws.write_merge(0, 0, 46, 53, 'Billing details', font_style)
        ...
        columns = ['Connection ID',
                   ...
                   'Customer Contact Person Number', 'Assign Noc Engineer',
                   'Assign Status', 'Sign Verification ID', 'Sign Verification Time',
                   'Assigned Date and time',
                   'ILL IP', 'Ill Subnet',
                   'Ill Gateway', 'Ill DNS',
                   'ILL DNS2', 'Ill Bandwidth',
        ...
        for objfeasible_1 in objfeasible:
            rows.append([objfeasible_1.conn_code.ill_custid if objfeasible_1.conn_code else '',
                         ...
                         objfeasible_1.sale_code.assign_noc.username if objfeasible_1.sale_code else '',
    ...
    return render(request, 'ttvapptemp/ttv_app_temp.html', context)


@login_required(login_url='login')
def addttvillcon(request):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newform.save()
            if newform.ill_conn_type == '2':
                if newform.ill_feasibility:
                    doneconn_obj = TTVIllDoneConnectionsDetails.objects.create(conn_code=newform, )
                    doneconn_obj.save()
                    if newform.assign_status:
                        salenotification_obj = IllNotification.objects.create(
                            ...
                            accept_mod_url="addttvillsales", reject_mod_url="rejectttvillsales", ill_app_name="2")
                        salenotification_obj.save()

            if newform.ill_feasibility:
                return redirect('ttvillconnections')
            else:
                return redirect('threesaillconnectionsoverallleads')
    context = {'form': form, 'objntfc_all': objntfc_all, 'objmsgs': objmsg}
    return render(request, 'ttvapptemp/add_ill_connection.html', context)


@login_required(login_url='login')
def ttvillsalesdetails(request):
    return redirect('ttvillconnections')


@login_required(login_url='login')
def addttvillsales(request, asnid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newform.save()
            doneconn_obj = TTVIllDoneConnectionsDetails.objects.get(conn_code=newform.conn_code)
            ...
            if newform.assign_status:
                nocnotification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addttvillnoc", reject_mod_url="rejectttvillnoc", ill_app_name="2")
    ...
    return render(request, 'ttvapptemp/add_ill_sales.html', context)


@login_required(login_url='login')
def editttvillsales(request, esid):
    etlsobj = TTVIllSalesDetails.objects.get(id=esid)
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                if not etlsobj.assign_status:
                    etlsobj.assign_noc = newform.assign_noc
                    if newform.assign_status:
                ...
                if newform.assign_status:
                    nocnotification_obj = IllNotification.objects.create(
                        ...
                        ill_ntfc_receiver=etlsobj.assign_noc.username, assign_date_time=etlsobj.ill_sign_time,
                        accept_mod_url="addttvillnoc", reject_mod_url="rejectttvillnoc", ill_app_name="2")
                    nocnotification_obj.save()
    ...
    return render(request, 'ttvapptemp/edit_ill_sales.html', context)


@login_required(login_url='login')
def rejectttvillsales(request, rsid):
    ...
    delobj = objsn.id
    objsn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'ttvillsalesdetails' in request.META.get(
            'HTTP_REFERER'):
...
@login_required(login_url='login')
def ttvillnocdetails(request):
    return redirect('ttvillconnections')


@login_required(login_url='login')
def addttvillnoc(request, anid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
        ...
        if form.is_valid():
            ...
            if newform.assign_status:
                fieldengg_notification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addttvillfieldengg", reject_mod_url="rejectttvillfieldengg", ill_app_name="2")
    ...
    return render(request, 'ttvapptemp/add_ill_noc.html', context)


@login_required(login_url='login')
def editttvillnoc(request, enid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlnobj = TTVIllNocDetails.objects.get(id=enid)
                ...
                if not etlnobj.assign_status:
                    ...
                    if newform.assign_status:
                        ...
                        fieldengg_notification_obj = IllNotification.objects.create(
                            ...
                            accept_mod_url="addttvillfieldengg", reject_mod_url="rejectttvillfieldengg",
                            ill_app_name="3")
                        fieldengg_notification_obj.save()
    ...
    return render(request, 'ttvapptemp/edit_ill_noc.html', context)


@login_required(login_url='login')
def rejectttvillnoc(request, rnid):
    objnn = IllNotification.objects.get(id=rnid)
    objillsales = TTVIllSalesDetails.objects.get(id=objnn.ill_prev_assign_obj_id)
    objillsales.assign_status = False
    objillsales.save()

    rejmsgobj = RejectionMessage.objects.create(msg_sender=request.user.username, msg_receiver=objnn.ill_ntfc_sender,
                                                conn_code=objillsales.conn_code.ill_custid, rej_module_name='Noc')
    rejmsgobj.save()

    delobj = objnn.id
    objnn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'ttvillnocdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def ttvillfieldenggdetails(request):
    return redirect('ttvillconnections')


@login_required(login_url='login')
def addttvillfieldengg(request, afeid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
        ...
        if form.is_valid():
            ...
            if newform.assign_status:
                billing_notification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addttvillbilling", reject_mod_url="rejectttvillbilling", ill_app_name="2")
    ...
    return render(request, 'ttvapptemp/add_ill_fieldengg.html', context)


@login_required(login_url='login')
def editttvillfieldengg(request, efeid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlfeobj = TTVIllFieldEngineerDetails.objects.get(id=efeid)
                ...
                if not etlfeobj.assign_status:
                    ...
                    if newform.assign_status:
                        ...
                        billing_notification_obj = IllNotification.objects.create(
                            ...
                            accept_mod_url="addttvillbilling", reject_mod_url="rejectttvillbilling", ill_app_name="3")
                        billing_notification_obj.save()
    ...
    return render(request, 'ttvapptemp/edit_ill_fieldengg.html', context)


@login_required(login_url='login')
def rejectttvillfieldengg(request, rfeid):
    objfen = IllNotification.objects.get(id=rfeid)
    objillnoc = TTVIllNocDetails.objects.get(id=objfen.ill_prev_assign_obj_id)
    ...
    delobj = objfen.id
    objfen.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'ttvillfieldenggdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def ttvillbillingdetails(request):
    return redirect('ttvillconnections')


@login_required(login_url='login')
def addttvillbilling(request, abid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
    ...
    return render(request, 'ttvapptemp/add_ill_billing.html', context)


@login_required(login_url='login')
def editttvillbilling(request, ebid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlbobj = TTVIllBillingDetails.objects.get(id=ebid)
    ...
    if request.user.username == etlbobj.ill_billing_receiver or request.user.is_staff:
        ...
        EditPer = True
        context = {'form': form, 'objntfc_all': objntfc_all, 'etlbobj': etlbobj, 'EditPer': EditPer,
                   'objmsgs': objmsg}
    ...
    return render(request, 'ttvapptemp/edit_ill_billing.html', context)


@login_required(login_url='login')
def rejectttvillbilling(request, rbid):
    objbn = IllNotification.objects.get(id=rbid)
    objillfieldengg = TTVIllFieldEngineerDetails.objects.get(id=objbn.ill_prev_assign_obj_id)
    ...
    delobj = objbn.id
    objbn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'ttvillbillingdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def renewttvillbilling(request, rebid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    retlbobj = TTVIllBillingDetails.objects.get(id=rebid)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if newform.payment_method == '2':
                ...
                print('ok', newform.netbanking_type)
            else:
                print('mm', newform.netbanking_type)
            ...
            doneconn_obj = TTVIllDoneConnectionsDetails.objects.get(conn_code=retlbobj.conn_code)
            if newform.conn_start_date <= date.today():
                doneconn_obj.active_status = True
            doneconn_obj.save()
    ...
    return render(request, 'ttvapptemp/renew_ill_billing.html', context)
