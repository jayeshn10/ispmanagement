...
class VTTIllFeasibleConnDetailsFilter(django_filters.FilterSet):
    LSTATUS_CHOICES = (
        (1, 'active'),
        (0, 'deactive'),
    )
    ...
    conn_code__ill_vendor_code = django_filters.ModelChoiceFilter(queryset=IllVendors.objects.all(), widget=forms.Select(
        attrs={'class': 'mr-2 form-control'}), empty_label='Select Vendor')

    class Meta:
        model = VTTIllDoneConnectionsDetails
        fields = ['conn_code', ]
