...
from vendorsillthroughthreesa.filters import VTTIllFeasibleConnDetailsFilter
from vendorsillthroughthreesa.forms import AddIllConForm, AddSalesDetailsForm, EditSalesDetailsForm, \
...
def vttillconnections(request):
    ...
    objfeasible = VTTIllDoneConnectionsDetails.objects.all()

    connfilter = VTTIllFeasibleConnDetailsFilter(request.GET, queryset=objfeasible)
    objfeasible = connfilter.qs

    if request.method == 'GET' and request.GET.get('searchqry'):
        searchguery = request.GET.get('searchqry')
        objfeasible = VTTIllDoneConnectionsDetails.objects.filter(Q(conn_code__ill_custid__icontains=searchguery)
                                                                  | Q(
            ...
            conn_code__ill_cust_address__icontains=searchguery)
                                                                  ...
                                                                  | Q(pkg_name__pkg_name__icontains=searchguery)
                                                                  | Q(pkg_startdate__icontains=searchguery)
                                                                  | Q(pkg_enddate__icontains=searchguery)
                                                                  | Q(active_status__icontains=searchguery)
                                                                  | Q(
            conn_code__ill_vendor_code__v_code__icontains=searchguery)
                                                                  )

    if request.method == 'POST' and request.POST.get('exportexcel'):
        response = HttpResponse(content_type='application/ms-excel')
        response['Content-Disposition'] = 'attachment; filename="VTT-Ill-done-Connections.xls"'

        wb = xlwt.Workbook(encoding='utf-8')
        ws = wb.add_sheet('VTT Ill Done Connections')  # this will make a sheet named Users Data
        ...
        ws.write_merge(0, 0, 0, 13, 'Connection Details', font_style)
        ws.write_merge(0, 0, 14, 23, 'Sales details', font_style)
        ws.write_merge(0, 0, 24, 31, 'Fiber details', font_style)
        ws.write_merge(0, 0, 32, 47, 'NOC details', font_style)
        ws.write_merge(0, 0, 48, 55, 'Field Engineer details', font_style)
        ws.write_merge(0, 0, 56, 63, 'Billing details', font_style)
        ...
        columns = ['Connection ID',
                   ...
                   'Sign Verification Time',
                   'Connection Activation Date', 'Connection Expiry Date', 'Connection Validity', 'Link status',
        ...
        for objfeasible_1 in objfeasible:
            rows.append([objfeasible_1.conn_code.ill_custid if objfeasible_1.conn_code else '',
                         objfeasible_1.conn_code.ill_vendor_code.v_code if objfeasible_1.conn_code else '',
    ...
    return render(request, 'vttapptemp/vtt_app_temp.html', context)


@login_required(login_url='login')
def addvttillcon(request):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newform.save()
            if newform.ill_conn_type == '3':
                if newform.ill_feasibility:
                    doneconn_obj = VTTIllDoneConnectionsDetails.objects.create(conn_code=newform, )
                    doneconn_obj.save()
                    if newform.assign_status:
                        salenotification_obj = IllNotification.objects.create(
                            ...
                            accept_mod_url="addvttillsales", reject_mod_url="rejectvttillsales", ill_app_name="3")
                        salenotification_obj.save()

            if newform.ill_feasibility:
                return redirect('vttillconnections')
            else:
                return redirect('threesaillconnectionsoverallleads')
    context = {'form': form, 'objntfc_all': objntfc_all, 'objmsgs': objmsg}
    return render(request, 'vttapptemp/add_ill_connection.html', context)


@login_required(login_url='login')
def vttillsalesdetails(request):
    return redirect('vttillconnections')


@login_required(login_url='login')
def addvttillsales(request, asnid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    objsn = IllNotification.objects.get(id=asnid)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            newform.save()
            doneconn_obj = VTTIllDoneConnectionsDetails.objects.get(conn_code=newform.conn_code)
            ...
            if newform.assign_status:
                fibernotification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addvttillfiber", reject_mod_url="rejectvttillfiber", ill_app_name="3")
    ...
    return render(request, 'vttapptemp/add_ill_sales.html', context)


@login_required(login_url='login')
def editvttillsales(request, esid):
    etlsobj = VTTIllSalesDetails.objects.get(id=esid)
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
    ...
    return render(request, 'vttapptemp/edit_ill_sales.html', context)


@login_required(login_url='login')
def rejectvttillsales(request, rsid):
    objsn = IllNotification.objects.get(id=rsid)
    ...
    rejmsgobj = RejectionMessage.objects.create(msg_sender=request.user.username,
                                                msg_receiver=objsn.ill_ntfc_sender,
                                                conn_code=objillcon.ill_custid, rej_module_name='sales')
    rejmsgobj.save()
    delobj = objsn.id
    objsn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'vttillsalesdetails' in request.META.get(
            'HTTP_REFERER'):
...
@login_required(login_url='login')
def vttillfiberdetails(request):
    return redirect('vttillconnections')


@login_required(login_url='login')
def addvttillfiber(request, afid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
        ...
        if form.is_valid():
            ...
            if newform.assign_status:
                nocnotification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addvttillnoc", reject_mod_url="rejectvttillnoc", ill_app_name="3")
    ...
    return render(request, 'vttapptemp/add_ill_fiber.html', context)


@login_required(login_url='login')
def editvttillfiber(request, efid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlfobj = VTTIllFiberTeam.objects.get(id=efid)
    ...
    return render(request, 'vttapptemp/edit_ill_fiber.html', context)


@login_required(login_url='login')
def rejectvttillfiber(request, rfid):
    objfn = IllNotification.objects.get(id=rfid)
    objillsale = VTTIllSalesDetails.objects.get(id=objfn.ill_prev_assign_obj_id)
    ...
    delobj = objfn.id
    objfn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'vttillfiberdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def vttillnocdetails(request):
    return redirect('vttillconnections')


@login_required(login_url='login')
def addvttillnoc(request, anid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
        ...
        if form.is_valid():
            ...
            if newform.assign_status:
                fieldengg_notification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addvttillfieldengg", reject_mod_url="rejectvttillfieldengg", ill_app_name="3")
    ...
    return render(request, 'vttapptemp/add_ill_noc.html', context)


@login_required(login_url='login')
def editvttillnoc(request, enid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlnobj = VTTIllNocDetails.objects.get(id=enid)
                ...
                if not etlnobj.assign_status:
                    ...
                    if newform.assign_status:
                        ...
                        fieldengg_notification_obj = IllNotification.objects.create(
                            ...
                            accept_mod_url="addvttillfieldengg", reject_mod_url="rejectvttillfieldengg",
                            ill_app_name="3")
                        fieldengg_notification_obj.save()
    ...
    return render(request, 'vttapptemp/edit_ill_noc.html', context)


@login_required(login_url='login')
def rejectvttillnoc(request, rnid):
    objnn = IllNotification.objects.get(id=rnid)
    objillfiber = VTTIllFiberTeam.objects.get(id=objnn.ill_prev_assign_obj_id)
    ...
    delobj = objnn.id
    objnn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'vttillnocdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def vttillfieldenggdetails(request):
    return redirect('vttillconnections')


@login_required(login_url='login')
def addvttillfieldengg(request, afeid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
        ...
        if form.is_valid():
            ...
            if newform.assign_status:
                billing_notification_obj = IllNotification.objects.create(
                    ...
                    accept_mod_url="addvttillbilling", reject_mod_url="rejectvttillbilling", ill_app_name="3")
    ...
    return render(request, 'vttapptemp/add_ill_fieldengg.html', context)


@login_required(login_url='login')
def editvttillfieldengg(request, efeid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlfeobj = VTTIllFieldEngineerDetails.objects.get(id=efeid)
    ...
    return render(request, 'vttapptemp/edit_ill_fieldengg.html', context)


@login_required(login_url='login')
def rejectvttillfieldengg(request, rfeid):
    objfen = IllNotification.objects.get(id=rfeid)
    objillnoc = VTTIllNocDetails.objects.get(id=objfen.ill_prev_assign_obj_id)
    ...
    delobj = objfen.id
    objfen.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'vttillfieldenggdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def vttillbillingdetails(request):
    return redirect('vttillconnections')


@login_required(login_url='login')
def addvttillbilling(request, abid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)
    ...
    return render(request, 'vttapptemp/add_ill_billing.html', context)


@login_required(login_url='login')
def editvttillbilling(request, ebid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    etlbobj = VTTIllBillingDetails.objects.get(id=ebid)
    ...
    return render(request, 'vttapptemp/edit_ill_billing.html', context)


@login_required(login_url='login')
def rejectvttillbilling(request, rbid):
    objbn = IllNotification.objects.get(id=rbid)
    objillfieldengg = VTTIllFieldEngineerDetails.objects.get(id=objbn.ill_prev_assign_obj_id)
    ...
    delobj = objbn.id
    objbn.delete()
    if str(delobj) in request.META.get('HTTP_REFERER') and 'vttillbillingdetails' in request.META.get(
            'HTTP_REFERER'):
        return redirect('index')
    else:
        return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
...
@login_required(login_url='login')
def renewvttillbilling(request, rebid):
    objntfc_all = IllNotification.objects.filter(ill_ntfc_receiver=request.user.username)
    objmsg = RejectionMessage.objects.filter(msg_receiver=request.user.username)

    retlbobj = VTTIllBillingDetails.objects.get(id=rebid)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            doneconn_obj = VTTIllDoneConnectionsDetails.objects.get(conn_code=retlbobj.conn_code)
            if newform.conn_start_date <= date.today():
                doneconn_obj.active_status = True
            doneconn_obj.save()
    ...
    return render(request, 'vttapptemp/renew_ill_billing.html', context)
