...
class ThreesaIllFeasibleConnDetailsFilter(django_filters.FilterSet):
    LSTATUS_CHOICES = (
        (1, 'active'),
        (0, 'deactive'),
    )
    ...
    castrd = django_filters.DateFilter(field_name='billing_code__conn_start_date', lookup_expr='gte', widget=forms.HiddenInput(
        attrs={'id': 'castrd', }))

    caendd = django_filters.DateFilter(field_name='billing_code__conn_end_date', lookup_expr='lte', widget=forms.HiddenInput(
        attrs={'id': 'caendd', }))

    cestrd = django_filters.DateFilter(field_name='billing_code__conn_end_date', lookup_expr='gte', widget=forms.HiddenInput(
        attrs={'id': 'cestrd', }))

    ceendd = django_filters.DateFilter(field_name='billing_code__conn_end_date', lookup_expr='lte', widget=forms.HiddenInput(
        attrs={'id': 'ceendd', }))
    ...
    class Meta:
        model = ThreesaIllDoneConnectionsDetails
        fields = ['conn_code', ]
