...
from sitemanager.models import User, ThreesaIllConnectionsDetails


class ThreesaIllSalesDetails(models.Model):
    ...
    def thumbnail_preview(self):
        if self.po_img:
            return mark_safe('<img src="{}" width="100" height="100" />'.format(self.po_img.url))
        return ""
...
class ThreesaIllFiberTeam(models.Model):
    MEDIA_CHOICES = (
        ('1', '1000'),
        ('2', '100'),
    )
...
class ThreesaIllNocDetails(models.Model):
    ROUTING_CHOICES = (
        ('1', 'done'),
        ('2', 'pending'),
    )
...
class ThreesaIllFieldEngineerDetails(models.Model):
    linkstatus = (
        ('1', 'up'),
        ('2', 'down'),
    )
    ...
    connectivity_img = models.FileField(verbose_name='Upload Connectivity Proof',
                                        upload_to='illdata/connectivityproofimages/',
                                        blank=True)
...
class ThreesaIllBillingDetails(models.Model):
    paymentstatus = (
        ('1', 'Done'),
        ('2', 'Pending'),
    )
...
class ThreesaIllDoneConnectionsDetails(models.Model):
    linkstatus = (
        (True, 'Active'),
        (False, 'Deactive'),
    )
    ...
    sale_code = models.ForeignKey(ThreesaIllSalesDetails, models.SET_NULL, verbose_name='sales details', blank=True,
                                  null=True)
    fiber_code = models.ForeignKey(ThreesaIllFiberTeam, models.SET_NULL, verbose_name='Fiber details', blank=True,
                                   null=True)
    noc_code = models.ForeignKey(ThreesaIllNocDetails, models.SET_NULL, verbose_name='NOC details', blank=True,
                                 null=True)
    fe_code = models.ForeignKey(ThreesaIllFieldEngineerDetails, models.SET_NULL, verbose_name='FE details', blank=True,
                                null=True)
    billing_code = models.ForeignKey(ThreesaIllBillingDetails, models.SET_NULL, verbose_name='billing details',
                                     blank=True, null=True)
    ...
    class Meta:
        verbose_name_plural = '1. Threesa ILL Done Connections'
